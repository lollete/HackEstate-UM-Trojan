...
import GeopFence from '@/components/admin/geo';
// Props interface
type Props = {
    property: Property;
};
...
const ListingView: React.FC<Props> = ({ property }) => {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        { title: 'Listings', href: '/view' },
        { title: property.name, href: window.location.pathname },
    ];

    const getMapEmbedUrl = (location: string) => `https://maps.google.com/maps?q=${encodeURIComponent(location)}&t=&z=13&ie=UTF8&iwloc=&output=embed`;
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 p-4 sm:p-6 lg:p-8">
                        ...
                        <div className="flex flex-col">
                            <img
                                ...
                                onError={(e) => (e.currentTarget.src = 'https://placehold.co/800x400/cccccc/333333?text=Main+Image')}
                            />
                            <div className="space-x-2 flex">
                                {[1, 2, 3].map((_, i) => (
                                    <img
                                        key={i}
                                        src="https://i.pinimg.com/1200x/00/01/44/00014489ecd72916d18d100471ccf901.jpg"
                                        alt={`Gallery ${i + 1}`}
                        ...
                        <div>
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 text-gray-700 text-sm mb-6">
                                <div className="flex items-center"><Calendar className="w-4 h-4 mr-1 text-gray-500" /> Built in 2005</div>
                                <div className="flex items-center"><BedDouble className="w-4 h-4 mr-1 text-gray-500" /> {property.bedrooms} Bedrooms</div>
                                <div className="flex items-center"><Bath className="w-4 h-4 mr-1 text-gray-500" /> {property.bathrooms} Bathrooms</div>
                            ...
                            <div className="flex space-x-4 mb-6">
                                ...
                                <button className="flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition"><Bookmark className="w-5 h-5 mr-2" /> Bookmark</button>
                                <button className="flex items-center px-4 py-2 bg-gray-100 text-gray-700 rounded-lg hover:bg-gray-200 transition"><Share2 className="w-5 h-5 mr-2" /> Share</button>
                            </div>
                            ...
                            <div className="border rounded-sm px-5 py-4 border-gray-200">
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center">
                                        ...
                                        />
                                        <div>
                                            <p className="text-lg font-semibold text-gray-900 flex items-center">{property.agent.name} <Eye className="text-[6px] text-white fill-green-700 ms-2" /></p>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                {property.details?.features && (
                                    <div>
                                        ...
                                        <ul className="list-disc list-inside text-gray-700 space-y-1">
                                            {property.details.features.map((feature, i) => <li key={i}>{feature}</li>)}
                                        </ul>
                                    </div>
                                )}
